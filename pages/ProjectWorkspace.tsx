...
const getClient = (customApiKey?: string) => {
  ...
  if (!apiKey || apiKey === 'undefined' || apiKey === '') {
    ...
    throw new Error("API Key is missing. 请在 API 配置中填入密钥，或检查后台环境变量。");
  }
...
const handleApiError = (error: any, defaultMsg: string): never => {
  ...
  if (msg.includes('400')) {
      throw new Error("请求参数无效 (400)。请检查提示词内容。");
  }
  if (msg.includes('503')) {
...
// Fixed: Update model name to gemini-3-flash-preview
export const generateText = async (prompt: string, customApiKey?: string, modelName: string = 'gemini-3-flash-preview'): Promise<string> => {
  try {
